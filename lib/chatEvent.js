  ...
  if (msg.message) {
    ...
    try {
      ...
      if (quoted.quotedMessage) {
        const quotedMessageType = Object.keys(quoted.quotedMessage)[0];
        if (quotedMessageType === "ephemeralMessage") {
          const innerType = Object.keys(quoted.quotedMessage.ephemeralMessage.message)[0];
...
module.exports = chatEvent = async (m, client) => {
  ...
  commands.forEach(async (command) => {
    ...
    const executeCommand = async () => {
      try {
        await command.function(msg, msg.body.replace(command.pattern, ''), client);
        runned = true;
      } catch (e) {
        console.log(e);
      }
    ...
    if (command.pattern && msg.body.toLowerCase().startsWith(command.pattern)) {
      if (command.fromMe && !admins.includes(sender)) {
        await msg.reply("This Command Can Only Be Used By The *Creator Of The Bot*");
        return;
      }
  ...
  // Check if the command should be run on all messages
