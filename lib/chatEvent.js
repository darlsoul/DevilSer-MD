...
function serialize(msg, client) {
  ...
  if (msg.message) {
    ...
    msg.mentions = msg.message[msg.type]?.contextInfo?.mentionedJid || null;
    ...
    try {
      ...
      if (quoted) {
        if (quoted["ephemeralMessage"]) {
          ...
          msg.quoted = {
            ...
            participant: decodeJid(msg.message[msg.type].contextInfo.participant),
            message: quoted.ephemeralMessage.message[type === "viewOnceMessage" ? "viewOnceMessage" : type].message,
          };
        ...
        msg.quoted.mtype = Object.keys(msg.quoted.message).find(v => v.includes("Message") || v.includes("conversation"));
      ...
      console.error('Error handling quoted message:', e);
      msg.quoted = null;
    }
    ...
    msg.send = async (content, options = {}, type = 'text') => {
      if (type !== 'text' && !Buffer.isBuffer(content) && typeof content !== 'object') {
...
function printLog(cmdName, pushName, sender, gcName, isGc) {
  if (isGc) {
    console.log("\n" + chalk.white.bgBlue(`[${formattedTime}]`) +
      chalk.black.bgGreen(` ${cmdName} From ${pushName} : ${sender.split("@")[0]} In ${gcName}`));
  } else {
    console.log("\n" + chalk.white.bgBlue(`[${formattedTime}]`) +
      chalk.black.bgGreen(` ${cmdName} From ${pushName} : ${sender.split("@")[0]}`));
  }
}
...
module.exports = chatEvent = async (m, client) => {
  ...
  let msg = serialize(m.messages[0], client);
  ...
  msg.body = msg.body.startsWith(HANDLERS) ? msg.body.replace(HANDLERS, '').trim() : false;
